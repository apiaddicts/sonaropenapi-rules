...
public class OAR039StandardResponseCodesCheck extends AbstractVerbPathCheck {
    ...
    private static final String GET_COLLECTION_CODES = OK_CODE + OR_OPERATOR + PARTIAL_CONTENT_CODE + VALUE_SEPARATOR
            + BAD_REQUEST_CODE + VALUE_SEPARATOR + COMMON_CODES;
    private static final String GET_SUBCOLLECTIONS_CODES = GET_COLLECTION_CODES + VALUE_SEPARATOR + NOT_FOUND_CODE;
    private static final String GET_ONE_CODES = OK_CODE + VALUE_SEPARATOR + BAD_REQUEST_CODE + VALUE_SEPARATOR
            + NOT_FOUND_CODE + VALUE_SEPARATOR + COMMON_CODES;
    private static final String POST_CODES = OK_CODE + OR_OPERATOR + CREATED_CODE + OR_OPERATOR + ACCEPTED_CODE
            + VALUE_SEPARATOR + OPS_WITH_BODY_CODES + VALUE_SEPARATOR + COMMON_CODES;
    private static final String POST_SUBRESOURCES_CODES = POST_CODES + VALUE_SEPARATOR + NOT_FOUND_CODE;
    private static final String UPDATE_N_OTHER_POST_OPS_CODES = OK_CODE + VALUE_SEPARATOR + NOT_FOUND_CODE
            + VALUE_SEPARATOR + OPS_WITH_BODY_CODES + VALUE_SEPARATOR + COMMON_CODES;
    private static final String DELETE_CODES = GET_ONE_CODES;

    private static final String DEFAULT_CODES_BY_RESOURCES_AND_VERB = GET_ALL_1ST_LEVEL + PART_SEPARATOR
    ...
    protected void matchV2(JsonNode node, JsonNode operationNode, String verb, String path, PatternGroup pg) {
        ...
        Set<String> currentCodes = responsesNode.properties().stream().map(JsonNode::key).map(JsonNode::getTokenValue)
        ...
        for (String missingCode : expectedCodes.stream().sorted().collect(Collectors.toList())) {
            ...
            if (orCodes.length > 1) {
                if (Collections.disjoint(currentCodes, Arrays.asList(orCodes))) {
                    addIssue(KEY,
                            translate(MESSAGE,
                                    String.join("en".equals(I18nContext.getLang()) ? " or " : " ó ", orCodes)),
                            responsesNode.key());
                }
            } else {
