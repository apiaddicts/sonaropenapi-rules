...
- OAR026 - TotalParameterDefaultValue - Rewritten to match Spectral exactly: subscribes to `GET` operations (was every `PARAMETER` node in the document) and only evaluates a `$total` parameter that is `in: query`, resolving `$ref`s (internal and external) along the way; a `$total` parameter under a non-`GET` operation, with a different `in`, or declared but never referenced by any `GET` is no longer flagged. A missing `default` is no longer reported (previously it was) - only an explicitly-declared non-`false` default raises an issue, matching Spectral's `falsy` semantics.
- OAR031 - Examples - Property-level checks (`validateProperty`) now only cover schemas actually reachable from a path's `parameters`/`requestBody`/`responses` (matching Spectral's `given` reach); an unreferenced `components.schemas`/`definitions` entry no longer produces false-positive property findings. Removed a hack that skipped every direct property of a schema with an `allOf` sibling key, and added `allOf`/`oneOf`/`anyOf` traversal to `isSchemaCovered` and the v2 response property walk, so a property declared only inside a combiner member (e.g. inherited via `allOf`) is now correctly checked (previously a false negative). Parameter- and property-level messages now interpolate the actual name (`Parameter '<name>' must have an example defined`, `Property '<name>' is missing an example.`), matching Spectral verbatim; response/request-body message text also corrected to match Spectral exactly (`Response must have an example defined`, `Request body must have an example defined`). Fixed the English rule doc listing the four toggle rule-property names in kebab-case (`validate-response`, ...) when the real keys are camelCase (`validateResponse`, ...). Also, found while cross-checking line numbers against Spectral: a `$ref`'d parameter's violation is now anchored at the shared definition's own key (e.g. `id:`) instead of its first content line, matching the response/request-body anchoring convention and Spectral's line; an OAS3 parameter example declared per media type under `content` is now recognized (previously always flagged as missing, regardless of a valid content-based example); the v2 response-driven property walk (`visitPathNode`) now excludes `204` responses, matching the primary walk; and Swagger 2.0 non-`body` parameters (query/path/header/formData) are now exempt from the parameter-level check in Spectral too (that field doesn't exist in the OAS2 parameter spec, only `default` does - Sonar already exempted them).
